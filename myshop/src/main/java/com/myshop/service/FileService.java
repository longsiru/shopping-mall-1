...
public class FileService {
	...
	public String uploadFile(String uploadPath, String originalFileName, byte[] fileDate) throws Exception{
		UUID uuid = UUID.randomUUID(); //종복되지 않은 랜덤한 파일이름 생성
		...
		//생성지에 파일이 저장될 위치와 파일 이름을 같이 넘겨 출력스트림 만든다
		FileOutputStream fos = new FileOutputStream(fileUploadFullUrl);
		fos.write(fileDate);//출력 스트림에 파일 데이터 입력
		fos.close();
	...
	public void deleteFile(String filePath) throws Exception{
		File deleteFile = new File(filePath); //파일 지정된 경로를 이용해서 파일 객체를 생성

...
class OrderTest {
	...
	public void cascadeTest() {
		...
		for(int i=0; i<3; i++){
			Item item = this.createItemTest(); //3개 물건 생성
	...
	@Test
	@DisplayName("고아객테 제거 테스트")
	public void orphanRemovalTest() {
	
		Order order = this.createOrder();
		order.getOrderItems().remove(0);  //주문 엔티티에서 주문상품 엔티티를 삭제 했을때 orderItem엔티티가 삭제가 돤다.
		em.flush();
	...
	public void lazyLoadingTest() {
		Order order = this.createOrder();
		Long orderItemId = order.getOrderItems().get(0).getId();//order_item table의 id
